...
The **Performance guardian** (on by default, opt-out in Settings) calms every decoration — ambient layers, fireflies, growth rituals, discovery reveals, living-garden animation — when memory pressure is high, while tracking, the companion, and your data always keep working. The guardian prefers the *real* free-system-memory signal (`chrome.system.memory`, read locally under the `system.memory` permission, never stored or transmitted; cached and refreshed on paths that already run — no new timers), and falls back to the browser's device-memory class and the extension context's own JS-heap use where that API is absent — the same per-renderer heap value memory-monitor extensions read via per-tab script injection, obtained here for free because the companion already lives in the page's renderer. The sensitivity bar (1–5) sets how early decorations rest, release hysteresis keeps the mode from flapping on noisy samples, and Settings shows a live readout of the exact signals the guardian sees.
